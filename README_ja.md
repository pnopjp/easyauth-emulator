...
→ [スタンドアロン セットアップ](#セットアップ) / [VS Code 拡張機能](vscode-extension/README_ja.md)
...
> **macOS x64（Intel）および Linux arm64** — これらのプラットフォーム向けのビルド済みバイナリは提供していません。使用するにはリポジトリをクローンし、`python scripts/package.py` を実行してビルドしてください。
...
デバッグセッションの開始・終了に連動してエミュレーターを自動起動・停止する VS Code 拡張機能が `vscode-extension/` に含まれています。インストールと設定については [vscode-extension/README_ja.md](vscode-extension/README_ja.md) を参照してください。
