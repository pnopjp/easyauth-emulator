...
PyInstaller を実行してローカル用アーカイブを作成します。Windows（`.zip`）、macOS・Linux（`.tar.gz`）に対応しています。

- **Windows**: amd64 のみ対応。arm64 は非対応（oauth2-proxy が Windows ARM 向けバイナリを配布していないため）。
- **クロスビルド非対応**: ビルド対象と同じ OS・アーキテクチャの環境で実行してください。
...
npm の依存パッケージをインストールしてください（初回のみ）:
