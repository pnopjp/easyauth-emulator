...
| 設定 | デフォルト | 説明 |
| --- | --- | --- |
| `easyauth.site.url` | `http://localhost` | 通常は変更不要。TLS 終端するフロント（トンネルドメインやリバースプロキシ）配下では `https://` の値を設定 |
...
既定の設定のまま動作します。ゲートウェイへは VS Code のポート転送を介して `http://localhost:<site.port>` でアクセスします。
...
2. `easyauth.site.url` にその URL を設定する（origin のみ・末尾スラッシュなし）。
3. IdP のアプリ登録のリダイレクト URI に `<転送 URL>/oauth2/callback` を追加する。

転送 URL はトンネルが存在する間は変わりませんが、トンネルを作り直すと変わります — 例: `code tunnel unregister` を実行した後や、未使用のトンネルが期限切れになった場合（既定では 30 日間アクティビティがないと削除）。その際は `easyauth.site.url` と IdP のリダイレクト URI を更新してください。トンネルの有効期間の詳細は [dev tunnels FAQ の「使用されていない開発トンネルはいつ削除されますか?」](https://learn.microsoft.com/ja-jp/azure/developer/dev-tunnels/faq) を参照してください。

同じワークスペースを Remote - SSH やローカル開発に戻す場合は、`easyauth.site.url` を既定値（`http://localhost`）に戻してください。
