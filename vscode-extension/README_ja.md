...
Microsoft Entra ID の Client ID とクライアントシークレットは設定済みですが、OIDC Issuer URL が未設定です。ステータスバーをクリックすると、ワークスペース設定の `easyauth.entra.oidcIssuerUrl` フィールドが開きます。テナント固有の Issuer URL（例: `https://login.microsoftonline.com/<テナントID>/v2.0`）を入力してください。
