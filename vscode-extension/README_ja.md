...
- **プライベートブラウザー起動** — `easyauth.privateBrowser.command` を設定すると、プライベート/シークレットウィンドウでゲートウェイ URL を開くボタンが表示される
...
| --- | --- |
| VS Code | 1.88 以降 |
| プラットフォーム | Windows x64、macOS（Apple Silicon）、macOS（Intel）、Linux x64、Linux arm64 |

> プラットフォームによりリリースのタイミングがずれる場合があります。特に **macOS（Intel）** 向けパッケージはビルド環境の制約により大幅に遅れる場合があります。対象プラットフォームの `.vsix` がまだ公開されていない場合は、リポジトリのルートで `python scripts/package.py --vsix` を実行してバイナリと `.vsix` をビルドし、**Extensions: Install from VSIX** でインストールしてください。
>
> **Windows arm64** — 非対応です。oauth2-proxy が Windows ARM 向けバイナリを配布していないため、サポートできません。
>
> **Web 版 VS Code（vscode.dev）** — この拡張機能はブラウザベースの VS Code 環境（vscode.dev・GitHub Codespaces の Web エディター）では動作しません。エミュレーターバイナリの実行にはネイティブ OS アクセスが必要です。
...
| 実行中 | ブラウザで開く、プライベートブラウザーで開く（設定済みの場合） |
...
| `EasyAuth Emulator: Open in Private Browser` | プライベート/シークレットウィンドウでゲートウェイ URL を開く（`easyauth.privateBrowser.command` 設定時のみ表示） |
...
| `easyauth.privateBrowser.command` | `""` | プライベート/シークレットウィンドウを起動するコマンド。サイト URL が末尾引数として付加される（例: `msedge --inprivate`、`chrome --incognito`、`firefox --private-window`）。空欄の場合はボタン非表示。 |
